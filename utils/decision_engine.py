...
Makes Coffee think and decide for himself when and how to respond.
"""
import random
import time
import json
...
class DecisionEngine:
    def __init__(self, api_key=None):
        ...
        self.active_topics = {}
        # Count messages since Coffee last spoke in each channel
    ...
    def should_respond(self, message_content, message=None, user_id=None, channel_id=None, reply_to_message_id=None):
        ...
        bot_name = "Coffee"  # Default fallback name
        try:
        ...
        # IMPROVEMENT: Better detect follow-up questions to Coffee's responses
        ...
        if len(content_lower.split()) <= 5 and channel_id:
            ...
            if is_question:
                # Import message tracker if available to check if last message was from Coffee
                try:
                    from bot import message_tracker
                    recent_messages = message_tracker.get_recent_channel_messages(channel_id, 3)
                    
                    if recent_messages and len(recent_messages) >= 1:
                        # Check if most recent message (other than current one) was from Coffee
                        for recent_msg in recent_messages:
                            ...
                            if bot_id_str and 'user_id' in recent_msg and recent_msg['user_id'] == bot_id_str:
                                print(f"Detected likely follow-up to Coffee's message: '{message_content}'")
        ...
        if is_replying_to_someone and not replying_to_bot and not bot_mentioned:
            if reply_target_name:
                return (False, f"Message is replying to {reply_target_name}, not to Coffee")
            else:
                return (False, "Message is replying to someone else, not to Coffee")
        
        # If message targets another user and NOT Coffee, don't respond
        if message_targets_user and not bot_mentioned:
            ...
            return (False, f"Message appears to be addressed to {target_user_name}, not Coffee")
        
        # If this is a reply to the bot, we should probably respond
        if replying_to_bot:
            return (True, "Message is a reply to Coffee's previous message")
            
        # If this is a likely follow-up question to Coffee's message, respond
        if is_short_followup:
            return (True, "Short follow-up question to Coffee's previous message")
            
        # Enhanced detection for follow-up questions about something Coffee mentioned
        # Check if this appears to be asking about something Coffee previously said
        if channel_id and "?" in content_lower and len(content_lower.split()) <= 15:
            try:
                ...
                # Look for words in the user's question that Coffee mentioned in previous messages
                ...
                # Look for overlap between Coffee's recent messages and the user question
                for sol_msg in sol_previous_msgs:
                    ...
                    # If there's significant overlap, this is likely about something Coffee said
                    if len(overlap) >= 1 and len(user_question_words) >= 1:
                        print(f"Detected follow-up about something Coffee mentioned. Overlapping terms: {overlap}")
                        return (True, "Question about something Coffee previously mentioned")
    ...
    def needs_internet_search(self, message_content, context):
        ...
        # Check if this is a follow-up question to something Coffee already said
        is_follow_up = False
        recent_bot_message = ""
        ...
        # If this looks like a follow-up to something Coffee said
        if is_short_question and recent_bot_message:
            ...
            try:
                ...
                RECENT CONVERSATION:
                Coffee: {recent_bot_message}
                User: {message_content}
                
                Based on this exchange:
                1. Is the user asking for specific details that weren't provided in Coffee's message?
                ...
                4. Does Coffee's previous response hint at knowledge that would require internet search to elaborate on?
    ...
    def check_moderation(self, message_content, user_id=None, channel_id=None, member=None):
        """
        Check if a message violates community rules using Gemini 2.0 model
        ...
        try:
            ...
            model = "google/gemini-2.0-flash-001"  # Default to Gemini for moderation
    ...
    def determine_response_type(self, message_content, context):
        ...
        try:
            ...
            headers = {
                ...
                "Content-Type": "application/json",
                "HTTP-Referer": "https://discord-bot.example.com",
                "X-Title": "Coffee Response Type Decision"
            }
    ...
    def should_wait_longer(self, message_content, is_burst):
        """
        Decide if Coffee should wait longer before responding
        ...
        Returns:
            bool: True if Coffee should wait longer
        """
