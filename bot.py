...
decision_engine = DecisionEngine(api_key=os.getenv('OPENROUTER_API_KEY'))  # Coffee's brain for autonomous decisions
...
async def on_message(message):
    ...
    # Check for moderator command with ".Coffee" prefix (more explicit than just "Coffee")
    ...
    if violates_rules:
        ...
        if should_warn:
            ...
            try:
                ...
                # This prevents Coffee from responding casually to someone they just warned
    ...
    # Special case for when the message starts with "Coffee" or "@Coffee"
    if message.content.lower().startswith("Coffee ") or message.content.lower().startswith("@Coffee "):
        print("BOT NAME PREFIX DETECTED")
        is_mentioned = True
        name_mentioned = True
    
    # Add message to tracker regardless - Coffee should always remember messages
    ...
    # ===== Coffee'S AUTONOMOUS DECISION PROCESS =====
    ...
    # If Coffee should wait and wasn't directly mentioned, don't respond yet
    ...
    # 4. Coffee has decided to respond - determine response style
    if force_respond:
        if replying_to_bot:
            print(f"RESPONDING TO MESSAGE! (Forced response: message is a reply to Coffee)")
        else:
    ...
    async with message.channel.typing():
        ...
        if response and response.strip():
            ...
            if message.reference and message.reference.message_id:
                ...
                should_use_reply = True
                print(f"Message is a reply to Coffee, using reply feature to maintain thread context")
            else:
                # Get recent messages in the channel
                recent_msgs = message_tracker.get_recent_channel_messages(message.channel.id, 10)
                
                # If there are messages between the current message and the last time Coffee responded
                # (someone else added messages after the user messaged Coffee), use reply mode
                if len(recent_msgs) >= 3:
                    # Look for patterns where: user message -> other messages -> Coffee's response
                    for i in range(len(recent_msgs) - 2):
                        ...
                        # If this sequence has: user message -> different user message -> (current Coffee response)
            ...
            if should_use_reply:
                # Use reply feature to clarify who Coffee is talking to
...
def is_admin(interaction: discord.Interaction):
    ...
    if member:
        ...
        admin_role_names = [
            ...
            # Add any other role names that should have admin access to Coffee
        ]
...
@bot.tree.command(name="help", description="Admin commands for Coffee")
async def slash_help(interaction: discord.Interaction):
    # Check if user is admin
    if not is_admin(interaction):
        await interaction.response.send_message("you need admin perms for that", ephemeral=True)
        return
    help_text = f"**Coffee admin commands**\n\n"
    
    help_text += f"**Channel Control:**\n"
    help_text += f"`/activate` - turn Coffee on in current channel\n"
    help_text += f"`/deactivate` - turn Coffee off in current channel\n"
    help_text += f"`/activateall` - turn Coffee on everywhere\n"
    help_text += f"`/deactivateall` - turn Coffee off everywhere\n"
    help_text += f"`/channels` - list active channels\n\n"
    
    help_text += f"**Settings:**\n"
    help_text += f"`/personality` - set Coffee's personality traits:\n"
    help_text += f"  • `chatty` - how often Coffee joins conversations (1-10)\n"
    help_text += f"  • `patience` - how long Coffee waits for complete thoughts (1-10)\n"
    help_text += f"  • `formality` - how formal Coffee's responses are (1-10)\n"
    help_text += f"`/setwindow [size]` - set how many messages Coffee remembers\n"
    help_text += f"`/setcasual [level]` - set how casual Coffee's replies are (1-10)\n"
    help_text += f"`/timing [min] [max]` - set typing delay in seconds\n\n"
    
    help_text += f"**Permissions:**\n"
    help_text += f"`/setcommandaccess [role] [add/remove]` - set roles that can use .Coffee commands\n"
    help_text += f"`/listcommandaccess` - list roles that can use .Coffee commands\n"
    ...
    help_text += f"`/spark [channel] [topic]` - make Coffee start a conversation in a quiet channel\n\n"
    
    help_text += f"**Natural Language Commands:**\n"
    help_text += f"Use `.Coffee` prefix for natural language commands, examples:\n"
    help_text += f"`.Coffee warn [user] for [reason]` - warn a user\n"
    help_text += f"`.Coffee mute [user] for [time] because [reason]` - timeout a user\n"
    help_text += f"`.Coffee say in #channel [message]` - send a message to a channel\n"
    help_text += f"`.Coffee search [query] and tell [user]` - search for information\n"
    help_text += f"`.Coffee open this as an issue on github: [issue]` - create a GitHub issue\n"
    help_text += f"`.Coffee create github issue in [org/repo]: [title]` - create an issue in a specific repo"
    
    await interaction.response.send_message(help_text, ephemeral=True)

@bot.tree.command(name="activate", description="Activate Coffee in a specific channel")
async def slash_activate(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(f"Coffee activated in {channel.mention}", ephemeral=True)

@bot.tree.command(name="deactivate", description="Deactivate Coffee in the current channel")
async def slash_deactivate(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(f"Coffee deactivated in {channel.mention}", ephemeral=True)

@bot.tree.command(name="activateall", description="Activate Coffee in all channels")
...
@bot.tree.command(name="deactivateall", description="Deactivate Coffee in all channels")
...
@bot.tree.command(name="status", description="Check Coffee's status and settings")
async def slash_status(interaction: discord.Interaction):
    ...
    status_text += f"command prefix: .Coffee\n\n"
...
@bot.tree.command(name="channels", description="List channels where Coffee is active")
async def list_channels(interaction: discord.Interaction):
    ...
    if not status['global_active']:
        await interaction.response.send_message("Coffee is currently off everywhere", ephemeral=True)
        return
    
    if not status['channel_mode']:
        await interaction.response.send_message("Coffee is active in all channels", ephemeral=True)
        return
    
    if len(status['active_channels']) == 0:
        await interaction.response.send_message("Coffee is not active in any channels", ephemeral=True)
        return
    
    channels_text = "**Coffee is active in:**\n"
...
# Settings commands
@bot.tree.command(name="setwindow", description="Set how many messages Coffee remembers")
...
async def set_context_window(interaction: discord.Interaction, size: int):
    ...
    await interaction.response.send_message(f"Coffee will now remember {size} messages", ephemeral=True)

@bot.tree.command(name="setcasual", description="Set how casual Coffee's responses are")
...
async def set_casualness(interaction: discord.Interaction, level: int):
    ...
    await interaction.response.send_message(f"Coffee's casualness level set to {level}/10", ephemeral=True)

@bot.tree.command(name="personality", description="Set Coffee's conversation style and AI behavior")
@app_commands.describe(
    chatty="How chatty Coffee should be (1-10, 1=responds rarely, 10=joins every conversation)",
    patience="How patient Coffee should be (1-10, 1=responds immediately, 10=waits for complete thoughts)",
    formality="How formal Coffee should be (1-10, 1=very casual, 10=extremely formal)"
)
async def set_personality(interaction: discord.Interaction, chatty: int = None, patience: int = None, formality: int = None):
    ...
    await interaction.response.send_message(f"updated Coffee's personality: {changes_text}", ephemeral=True)
...
async def set_timing(interaction: discord.Interaction, min_time: float, max_time: float):
    ...
    await interaction.response.send_message(f"Coffee's typing delay set to {min_time}-{max_time}s", ephemeral=True)
    
@bot.tree.command(name="reload", description="Reload Coffee's system prompt")
...
@bot.tree.command(name="spark", description="Make Coffee continue or start a conversation based on channel history")
@app_commands.describe(
    channel="The channel to continue a conversation in (defaults to current channel)",
    topic="Optional topic suggestion (leave empty for Coffee to analyze channel history)"
)
...
async def set_log_channel(
    ...
    if channel is not None:
        ...
        try:
            ...
            embed.set_footer(text="Coffee Moderation System")
...
async def process_natural_command(message):
    ...
    # Extract the command part (remove ".Coffee " prefix)
    command_text = message.content[5:].strip()
    
    # Create AI prompt to understand the command
    command_prompt = f"""
    You are a Discord bot named Coffee. A moderator has issued you a command in natural language. 
    ...
    try:
        if command_type == "WARNING":
            ...
            if target_user:
                ...
                if message_content:
                    # Use AI to rephrase in Coffee's casual style
                    try:
...
@bot.tree.command(name="setcommandaccess", description="Set which roles can use Coffee natural language commands")
@app_commands.describe(
    role="The role that can use '.Coffee' commands",
...
async def set_command_access(
    ...
    if action == "add":
        ...
        if role.id in command_roles:
            await interaction.response.send_message(f"Role '{role.name}' already has permission to use Coffee commands", ephemeral=True)
            return
        ...
        await interaction.response.send_message(f"Role '{role.name}' can now use Coffee commands", ephemeral=True)
    else:
        ...
        await interaction.response.send_message(f"Role '{role.name}' can no longer use Coffee commands", ephemeral=True)

@bot.tree.command(name="listcommandaccess", description="List roles that can use Coffee natural language commands")
async def list_command_access(interaction: discord.Interaction):
    ...
    if not command_roles:
        await interaction.response.send_message("No roles have been specifically allowed to use Coffee commands. Only users with manage_messages or administrator permissions can use them.", ephemeral=True)
        return
    ...
    # Create embed
    embed = discord.Embed(
        title="Roles That Can Use Coffee Commands",
        description="\n".join(role_names) if role_names else "No valid roles found",
        color=discord.Color.blue()
    )
